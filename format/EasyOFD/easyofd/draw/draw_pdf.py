...
"""
def safe_set_font(canvas, font_name, size):
    try:
        canvas.setFont(font_name, size)
    except Exception:
        fallback = 'SimSun' if any(c > '\u007f' for c in font_name) else 'Arial'
        print(f"⚠️ 字体 {font_name} 未注册，使用回退字体 {fallback}")
        canvas.setFont(fallback, size)
"""
def safe_set_font(canvas, font_name, size, text=""):
    """
    设置字体，如果字体未注册则按文本内容自动降级到中英文回退字体
    """
    try:
        canvas.setFont(font_name, size)
        ...
        # 用文本内容判断中英文
        fallback = 'SimSun' if any(ord(ch) > 127 for ch in text) else 'Arial'
        #logger.warning(f"⚠️ 字体 '{font_name}' 未注册，使用回退字体 '{fallback}'")
        try:
            canvas.setFont(fallback, size)
            #return fallback
        except Exception as e:
            #logger.error(f"回退字体 '{fallback}' 也未注册，使用默认字体 Times-Roman。异常: {e}")
...
def register_all_fonts():
    ...
    for root, _, files in os.walk(font_dirs[0]):
        for f in files:
...
#register_all_fonts()
#pdfmetrics.registerFont(TTFont('宋体', '/usr/share/fonts/truetype/arphic/uming.ttc'))
#pdfmetrics.registerFont(TTFont('黑体', '/usr/share/fonts/truetype/wqy/wqy-zenhei.ttc'))
#pdfmetrics.registerFont(TTFont('楷体', '/usr/share/fonts/truetype/arphic-gkai00mp/gkai00mp.ttf'))
#pdfmetrics.registerFont(TTFont('仿宋', '/usr/share/fonts/truetype/arphic/ukai.ttc'))
...
class DrawPDF():
    ...
    def __init__(self, data, *args, **kwargs):
        ...
        EasyOFD_path=os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
        uming_ttc_path = Path(EasyOFD_path) / 'fonts' / 'uming.ttc'
        pdfmetrics.registerFont(TTFont('宋体',str(uming_ttc_path)))
        pdfmetrics.registerFont(TTFont('SimSun',str(uming_ttc_path) ))
    ...
    def gen_empty_pdf(self):
        ...
        #safe_set_font(c,self.init_font, line_dict["size"] * self.OP,text)
    ...
    def draw_chars(self, canvas, text_list, fonts, page_size):
        """
        def safe_set_font(c, font_name, size):
           
            try:
                c.setFont(font_name, size)
            except Exception:
                # 根据字符判断回退中文还是英文
                fallback = 'SimSun' if any(ord(ch) > 127 for ch in font_name) else 'Arial'
                logger.warning(f"⚠️ 字体 '{font_name}' 未注册，使用回退字体 '{fallback}'")
                try:
        ...
        for line_dict in text_list:
            ...
            try:
                safe_set_font(c,font, line_dict["size"] * self.OP,text)
                    ...
                    # 按字符写入
                        ...
                        for cahr_id, _cahr_ in enumerate(text):
                            if len(x_list)>cahr_id:
                                # print("char wtite")
                                safe_set_font(c,font, line_dict["size"] * self.OP*resizeX,text)
        ...
        for key, annotation in annota_info.items():
            if annotation.get("AnnoType").get("type") == "Stamp":
                ...
                if not boundary or not isinstance(boundary, str):
                    continue
                pos = [float(i) for i in boundary.split(" ")]
                wrap_pos = annotation.get("Appearance", {}).get("Boundary", "")
                wrap_pos = [float(i) for i in wrap_pos.split(" ")] if wrap_pos else []
                ctm = img_obj.get("CTM", "")
                ctm = [float(i) for i in ctm.split(" ")] if ctm else []
