...
import json
from utils.client import stream_image_description, stream_text,stream_image_description_async,_collect_stream_async
import asyncio
...
async def analyze_image_content_async(
    ...
    异步版：desc 和 html 两次调用并行；单张图内部保持 分类 -> (desc || html) 的依赖顺序。
    """
    # ==========================================
    # 图片处理与编码
    # ==========================================
    ...
    try:
        raw_cls = await _collect_stream_async(client, base64_image, classify_prompt, model_name)
    except Exception as e:
        raise RuntimeError(f"分类阶段出错: {str(e)}")

    image_cls_result = raw_cls.strip().lower()
    ...
    # ==========================================
    # 第三步：desc 和 html 并行调用
    # ==========================================
    async def desc_api_call():
        try:
            return await _collect_stream_async(
                client, base64_image, img_title + desc_prompt, model_name
            )
        except Exception as e:
    ...
    async def html_api_call():
        try:
            return await _collect_stream_async(
                client, base64_image, html_prompt, model_name
            )
        except Exception as e:
            raise RuntimeError(f"html提取阶段出错: {str(e)}")

    need_desc = image_desc
    need_html = image_html and image_cls in {"line graph", "bar chart", "pie chart"}
...
def analyze_image_content(image_path, img_title, image_class: bool, image_desc: bool, image_html: bool, client, model_name):
    ...
    def cls_call(): # 重命名防止与外部变量冲突
        ...
        try:
            ...
            #   temperature=0.1,
            #)
            image_cls_result = answer.strip().lower()  # 使用流式输出的结果进行分类结果判断
