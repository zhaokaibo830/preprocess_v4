...
async def _analyze_image_content_impl(image_path, config, api_key, base_url, model_name):
    ...
    if image_cls == "pictogram":
        desc_prompt = (
            "你是一位专业的统计图表分析专家。请仔细分析这张统计图表,给出统计图表的相应描述,要点如下:提取图表的标题、横纵坐标含义和单位,以文字形式给出.描述数据的整体趋势(如上升、下降、波动),请确保你的回答简要精炼,不要包含额外的解释性文字。]\",\n"
        )
        html_prompt = (
            '请从统计图中尽可能准确地提取数据,并以规范的HTML表格形式呈现。例如:<table><tr><th>...</th>...</tr></table>,若无法提取,请说明'
        )
    else:  # other
        desc_prompt = (
            "你是一位通用的图片描述专家。请对这张图片进行简要但全面的描述,首先用简明的语言说明这是一张什么类型的图片(例如,'这是一张电路图','这是一张工人施工时的照片','这是一张产品效果图'等),描述图片中的主要主体(人物、物体、场景),以及它们在做什么或呈现出什么状态,如果图片中包含醒目的文字,请提取这些关键文字,并简要概括其传达的核心信息或功能\n"
        )
