...
def convert_json_format(json_data: Union[Dict, List]) -> List[Dict]:
    ...
    Args:
        json_data: 输入的JSON数据，可以是字典或列表格式
    ...
    if isinstance(json_data, dict):
        ...
        if "partitions" in json_data and "output" in json_data["partitions"]:
            original_data = json_data["partitions"]["output"]
...
# 为了保持向后兼容，保留原有的main函数
def main():
    """
    主函数：执行JSON格式转换（从文件读取）
    """
    # 配置输入输出文件路径
